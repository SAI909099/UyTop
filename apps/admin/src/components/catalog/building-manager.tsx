...
import { createBuilding, updateBuilding } from '@/lib/api/catalog';
import type { CatalogBuilding, ProjectLookupOption } from '@/types/api';
...
const emptyState: BuildingFormState = {
  selectedSlug: '',
  project_id: '',
  code: '',
  name: '',
...
export function BuildingManager({ projects, buildings, statuses }: BuildingManagerProps) {
  ...
  const [form, setForm] = useState<BuildingFormState>(emptyState);
          ...
          <div className="catalog-record-list">
            {items.map((building) => (
              ...
              >
                ...
                <span>{building.code} · {building.status}</span>
              </button>
            ))}
          </div>
        </div>
